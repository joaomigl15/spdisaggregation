...
def partition_spline_curve(alpha):
    ...
    x = tf.where(alpha < 4, (c(2.25) * alpha - c(4.5)) / (tf.abs(alpha - c(2)) + c(0.25)) + alpha + c(2),
                 c(5) / c(18) * log_safe(c(4) * alpha - c(15)) + c(8))
    return x
...
def lossfun(x, alpha, scale, approximate=False, epsilon=1e-6):
    ...
    if approximate:
        ...
        alpha_safe = tf.where(tf.greater_equal(alpha, 0.), tf.ones_like(alpha), -tf.ones_like(alpha)) * tf.maximum(
            machine_epsilon, tf.abs(alpha))
        loss_otherwise = (beta_safe / alpha_safe) * (tf.pow(squared_scaled_x / beta_safe + 1., 0.5 * alpha) - 1.)
        loss = tf.where(tf.equal(alpha, -tf.cast(float('inf'), float_dtype)), loss_neginf,
                        tf.where(tf.equal(alpha, 0.), loss_zero, tf.where(tf.equal(alpha, 2.), loss_two, tf.where(
                            tf.equal(alpha, tf.cast(float('inf'), float_dtype)), loss_posinf, loss_otherwise))))
    return loss
...
class Distribution(object):
    ...
    def log_base_partition_function(self, alpha):
        ...
        return interpolate1d(x * tf.cast(self._spline_x_scale, float_dtype), tf.cast(self._spline_values, float_dtype),
    ...
    def draw_samples(self, alpha, scale):
        ...
        def while_body(samples, accepted):
            ...
            nll_bound = lossfun(cauchy_sample, tf.cast(0, float_dtype), tf.cast(1, float_dtype),
...
class AdaptiveLossFunction(tf.Module):
    ...
    def __init__(self, num_channels, float_dtype=tf.float32, alpha_lo=0.001, alpha_hi=1.999, alpha_init=None,
                 scale_lo=1e-5, scale_init=1.0, name=None):
        ...
        if alpha_lo != alpha_hi:
            ...
            self._latent_alpha = tf.Variable(tf.fill((1, num_channels), tf.cast(latent_alpha_init, dtype=float_dtype)),
                                             name='LatentAlpha')
        if scale_lo != scale_init: self._latent_scale = tf.Variable(tf.zeros((1, num_channels), float_dtype),
                                                                    name='LatentScale')
    ...
    def alpha(self):
        if self._alpha_lo == self._alpha_hi:
            return tf.tile(tf.cast(self._alpha_lo, self._float_dtype)[tf.newaxis, tf.newaxis], (1, self._num_channels))
        else:
            return affine_sigmoid(self._latent_alpha, lo=self._alpha_lo, hi=self._alpha_hi)

    def scale(self):
        if self._scale_lo == self._scale_init:
            return tf.tile(tf.cast(self._scale_init, self._float_dtype)[tf.newaxis, tf.newaxis],
                           (1, self._num_channels))
        else:
            return affine_softplus(self._latent_scale, lo=self._scale_lo, ref=self._scale_init)
...
class AdaptiveImageLossFunction(tf.Module):
    ...
    def __call__(self, x):
        ...
        x_mat = tf.reshape(tf.transpose(tf.reshape(x_stack, [-1, num_channels, width, height]), perm=[0, 2, 3, 1]),
        ...
        if self._summarize_loss:
            ...
            tf.summary.image('robust/log_scale',
                             (log_scale[tf.newaxis] - log_scale_min) / (log_scale_max - log_scale_min + 1e-10))
